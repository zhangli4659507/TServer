...
- (void)calculateItemFrames {
    ...
    if (contentWidth < self.scrollView.frame.size.width) {
        ...
        CGFloat (^shiftDis)(int, CGFloat);
        switch (self.layoutMode) {
            case WMMenuViewLayoutModeScatter: {
                CGFloat gap = distance / (self.titlesCount + 1);
                shiftDis = ^CGFloat(int index, CGFloat width) { return gap * (index + 1); };
                break;
            }
            case WMMenuViewLayoutModeLeft: {
                shiftDis = ^CGFloat(int index, CGFloat width) { return 0.0; };
                break;
            }
            case WMMenuViewLayoutModeRight: {
                shiftDis = ^CGFloat(int index, CGFloat width) { return distance; };
                break;
            }
            case WMMenuViewLayoutModeCenter: {
                shiftDis = ^CGFloat(int index, CGFloat width) { return distance / 2; };
                break;
            }
            case WMMenuViewLayoutModeDivide: {
                CGFloat gap = self.scrollView.frame.size.width / self.titlesCount;
                shiftDis = ^CGFloat(int index, CGFloat width) {
                    return ((gap - width)/2.0 + index * gap);
                };
                break;
            }
        }
        for (int i = 0; i < self.frames.count; i++) {
            CGRect frame = [self.frames[i] CGRectValue];
            if (self.layoutMode == WMMenuViewLayoutModeDivide) {
                frame.origin.x = shiftDis(i, frame.size.width);
            }
            else {
                frame.origin.x += shiftDis(i, frame.size.width);
            }
