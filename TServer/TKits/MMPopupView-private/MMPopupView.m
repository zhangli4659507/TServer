...
- (void)showWithBlock:(MMPopupCompletionBlock)block
{
    ...
    /********************/
    // add by liqi
    for (UIView *sub in kCurrentWindow.subviews) {
    ...
    MMPopupBlock block = ^(MMPopupView *popupView){
        ...
        if ( !self.superview )
        {
            ...
            [self mas_updateConstraints:^(MASConstraintMaker *make) {
                ...
                if (@available(iOS 11.0, *)) {
                    make.bottom.equalTo(self.attachedView.mas_safeAreaLayoutGuideBottom).offset(self.attachedView.frame.size.height);
                } else {
        ...
        [UIView animateWithDuration:self.animationDuration
                         ...
                         animations:^{
                             ...
                             [self mas_updateConstraints:^(MASConstraintMaker *make) {
                                 if (@available(iOS 11.0, *)) {
                                     make.bottom.equalTo(self.attachedView.mas_safeAreaLayoutGuideBottom).offset(0);
                                 } else {
