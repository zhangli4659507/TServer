...
@property (nonatomic, weak) id<MMSheetViewDelegate> delegate;
@end

@implementation MMSheetView

- (instancetype) initWithTitle:(NSString*)title
                         items:(NSArray*)items delegate:(id<MMSheetViewDelegate>)delgate
{
    self = [super init];
    ...
    if ( self )
    {
        ...
        [self setContentCompressionResistancePriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisHorizontal];//横向抗压缩性强
        [self setContentHuggingPriority:UILayoutPriorityFittingSizeLevel forAxis:UILayoutConstraintAxisVertical];//纵向抗拉伸性差
...
- (void)actionButton:(UIButton*)btn
{
    ...
    if ([self.delegate respondsToSelector:@selector(sheetView:selectIndex:)]) {
        [self.delegate sheetView:self selectIndex:btn.tag];
    }
