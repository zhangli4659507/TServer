...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.square = NO;
        self.vPadding = kPadding;
        self.hPadding = kPadding;
...
- (void)layoutSubviews {
    ...
    CGFloat maxWidth = bounds.size.width - 2 * margin - 2 * _hPadding;
	...
	if (labelSize.height > 0.f && indicatorF.size.height > 0.f) {
        ...
        totalSize.height += _vPadding;
	}

	//CGFloat remainingHeight = bounds.size.height - totalSize.height - kPadding - 4 * margin;
    CGFloat remainingHeight = bounds.size.height - totalSize.height - _vPadding - 2 * margin - 2 * _vPadding;
    ...
    totalSize.width += 2 * _hPadding;
    totalSize.height += 2 * _vPadding;
	
	// Position elements
	//CGFloat yPos = round(((bounds.size.height - totalSize.height) / 2)) + margin + yOffset;
    CGFloat yPos = round(((bounds.size.height - totalSize.height) / 2)) + _vPadding + yOffset;
